...
class DongchediCrawler:
    ...
    def scrape_posts(self):
        ...
        for community_key, community_info in COMMUNITIES.items():
            ...
            total_pages = community_info.get(
                "total_pages", 1
            )  # Default to 1 page if not specified
            page_offset = community_info.get(
                "page_offset", 0
            )  # Default to 0 offset if not specified
            ...
            print(
                f"从社区 {community_key} 的第 {start_page_for_community + 1} 页开始抓取..."
            )
