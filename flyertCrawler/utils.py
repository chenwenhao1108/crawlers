...
def parse_timestamp(time_str):
    ...
    if date_match:
        try:
            return datetime.strptime(date_match.group(1), "%Y-%m-%d %H:%M").strftime(
                "%Y-%m-%d %H:%M"
            )
        except ValueError:
            pass
    ...
    if before_yesterday_match:
        try:
            ...
            return datetime.strptime(full_time, "%Y-%m-%d %H:%M").strftime(
                "%Y-%m-%d %H:%M"
            )
        except ValueError:
            pass
...
class ProgressManager:
    ...
    def is_link_processed(self, link):
        return (
            link in self.progress["processed_links"]
            or link in self.progress["error_links"]
            or link in self.progress["empty_links"]
        )
