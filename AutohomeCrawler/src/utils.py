            ...
            for cookie in cookies:
                ...
                try:
                    if "domain" in cookie and cookie["domain"]:
                        ...
                        if (
                            driver.current_url
                            and cookie["domain"] in driver.current_url
                        ):
...
def save_error_page(driver, url):
    ...
    try:
        ...
        filename_base = re.sub(
            r"[^a-zA-Z0-9_\-.]", "", url.rsplit("/", 1)[-1] if url else "error"
        )
        output_folder = "error_pages"
        os.makedirs(output_folder, exist_ok=True)

        file_path = os.path.join(
            output_folder, f"page_source_{filename_base}_{int(time.time())}.html"
        )
        with open(file_path, "w", encoding="utf-8") as file:
            file.write(
                str(soup.prettify())
            )  # Use str() for compatibility with type hints
