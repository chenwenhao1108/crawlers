...
class AutohomeCrawler:
    ...
    def _load_data_and_progress(self):
        ...
        for community_key in COMMUNITIES.keys():
            if community_key not in self.posts or not isinstance(
                self.posts[community_key], list
            ):
        ...
        if not isinstance(self.links, dict):
            self.links = {}
        for community_key in COMMUNITIES.keys():
            if community_key not in self.links or not isinstance(
                self.links[community_key], list
            ):
        ...
        print(
            f"加载完成：{len(self.posts)} 个社区的数据, {sum(len(v) for v in self.links.values())} 个链接, {len(self.processed_post_urls)} 条进度记录。"
        )
    ...
    def _ensure_login(self):
        ...
        if load_cookies(self.driver, COOKIES_FILE):
            ...
            time.sleep(
                2
            )  # Give some time for cookies to be applied and potential redirects
        ...
        for community_key, community_info in COMMUNITIES.items():
            ...
            for i in range(page_offset, total_pages + page_offset):
                ...
                try:
                    ...
                    print(
                        f"错误: 社区 {community_key} 的 URL 模板格式不正确: {url_template}"
                    )
                    continue  # Skip this community if URL template is invalid

                print(f"抓取社区 {community_key} 的第 {page_num} 页链接...")

                partial_links = get_post_detail_links(
                    self.driver, page_url, page_num, WAIT_TIME
                )
    ...
    def scrape_details(self):
        ...
        for community_key, links_list in self.links.items():
            if not isinstance(links_list, list):
                print(
                    f"警告: 社区 {community_key} 的链接数据格式不正确，跳过详情抓取。"
                )
                continue
                ...
                if link and link not in self.processed_post_urls:
                    ...
                    if post_detail:
                        ...
                        if processed_count_session % 10 == 0:  # Save every 10 posts
                            ...
                            print(
                                f"已抓取并保存 {processed_count_session} 个帖子详情 (总计 {len(self.processed_post_urls)})。"
                            )
                        ...
                        time.sleep(
                            random.uniform(*RANDOM_DELAY_RANGE)
                        )  # Still add delay even if failed
    ...
    def run(self, scrape_links=True, scrape_details=True):
        ...
        try:
            ...
            if scrape_details:
                ...
                if (
                    scrape_links
                ):  # Only reload if links were potentially updated in this run
                    self._load_data_and_progress()
                self.scrape_details()
